...
namespace wrapper
{
    public enum cs_enum
    {
        ...
        // There are not enough resources to correctly process the 
        // call.
        //
        // Possible explanations: it might be a lack of POSIX 
        // semaphores on systems where atomic locks are implemented 
        // that way or a failure in initializing a pthread_mutex (or on 
        // Windows, a critical section).
        ...
        // function.
        //
        // This invalid length will usually indicate that the length 
        ...
        // The process was not properly initialized. One possibility: 
        // was psoInit() called?
        ...
        // Socket error when trying to receive a reply from the 
        // watchdog.
        RECEIVE_ERROR = 28,

        // The shared-memory backstore file is missing. The name of 
        // this file is provided by the server (quasar) - if it is 
        // missing, something really weird is going on.
        BACKSTORE_FILE_MISSING = 29,

        // Generic i/o error when attempting to open the shared memory.
        ERROR_OPENING_MEMORY = 30,

        // Error accessing the directory for the log files or error 
        // opening the log file itself.
        LOGFILE_ERROR = 41,

        // Cannot get a lock on the session (a pthread_mutex or a 
        // critical section on Windows).
        SESSION_CANNOT_GET_LOCK = 42,

        // An attempt was made to use a session object (a session 
        // handle) after this session was terminated.
        ...
        // Attempt to create an object of an unknown object type or to 
        // open an object of the wrong type.
        WRONG_OBJECT_TYPE = 56,

        // Cannot get lock on the object. This might be the result of 
        // either a very busy system where unused cpu cycles are rare 
        // or a lock might be held by a crashed process.
        ...
        // The name of the object is too long. The maximum length of a 
        // name cannot be more than PSO_MAX_NAME_LENGTH (or 
        // PSO_MAX_FULL_NAME_LENGTH for the fully qualified name).
        OBJECT_NAME_TOO_LONG = 60,

        // You cannot delete a folder if there are still undeleted 
        // objects in it.
        //
        // Technical: a folder does not need to be empty to be deleted 
        // but all objects in it must be "marked as deleted" by the 
        // current session. This enables writing recursive deletions
        ...
        // The object is scheduled to be deleted soon. Operations on 
        ...
        // created and is not committed. Operations on this data item 
        ...
        // The object is scheduled to be deleted soon or was just 
        // created and is not committed. Operations on this object are 
        // not permitted at this time.
        OBJECT_IS_IN_USE = 69,

        // The object is read-only and update operations 
        // (delete/insert/replace) on it are not permitted. at this 
        // time.
        OBJECT_IS_READ_ONLY = 70,

        // All read-only objects open for updates (as temporary 
        // objects) must be closed prior to doing a commit on the 
        // session.
        NOT_ALL_EDIT_ARE_CLOSED = 71,

        // Read-only objects are not updated very frequently and 
        // therefore only a single editing copy is allowed. To allow 
        // concurrent editors (either all working on the same copy or 
        // each working with its own copy would have been be possible 
        // but was deemed unnecessary.
        A_SINGLE_UPDATER_IS_ALLOWED = 72,

        // The number of fields in the data definition is invalid - 
        // either zero or greater than PSO_MAX_FIELDS (defined in 
        // photon/psoCommon.h).
        INVALID_NUM_FIELDS = 101,

        // The data type of the field definition does not correspond to 
        // one of the data type defined in the enum psoFieldType 
        // (photon/psoCommon.h). or you've used PSO_VAR_STRING or 
        // PSO_VAR_BINARY at the wrong place.
        //
        // Do not forget that PSO_VAR_STRING and PSO_VAR_BINAR can only 
        // be used for the last field of your data definition.
        ...
        // characters are the standard ASCII alphanumerics 
        // ([a-zA-Z0-9]) and the underscore ('_'). The first character 
        // of the name must be letter.
        INVALID_FIELD_NAME = 105,

        // The name of the field is already used by another field in 
        // the current definition.
        //
        // Note: at the moment field names are case sensitive (for 
        // example "account_id" and "Account_Id" are considered 
        // different). This might be changed eventually so this 
        // practice should be avoided.
        DUPLICATE_FIELD_NAME = 106,

        // The precision of a PSO_DECIMAL field is either zero or over 
        // the limit for this type (set at 30 currently). Note: 
        // precision is the number of digits in a number.
        INVALID_PRECISION = 107,

        // The scale of a PSO_DECIMAL field is invalid (greater than 
        // the value of precision. Note: scale is the number of digits 
        // to the right of the decimal separator in a number.
        INVALID_SCALE = 108,

        // The key definition for a hash map is either invalid or 
        // missing.
        INVALID_KEY_DEF = 109,
        ...
        //
        // A simple Google search for "well-formed xml" returns many 
        // web sites that describe the syntax rules for XML. You can 
        // also use the program xmllint (included in the distribution 
        // of libxm2) to pinpoint the issue.
        XML_READ_ERROR = 201,
        ...
        // The root element must have an attribute named schemaLocation 
        // (in the namespace 
        // "http://www.w3.org/2001/XMLSchema-instance") to point to the 
        // schema use for the xml buffer stream.
        //
        // This attribute is in two parts separated by a space. The 
        // code expects the file name of the schema in the second 
        // element of this attribute.
        XML_NO_SCHEMA_LOCATION = 203,

        // The creation of a new schema parser context failed. There 
        // might be multiple reasons for this, for example, a 
        // memory-allocation failure in libxml2. However, the most 
        // likely reason is that the schema file is not at the location 
        // indicated by the attribute schemaLocation of the root 
        // element of the buffer stream.
        XML_PARSER_CONTEXT_FAILED = 204,

        // The parse operation of the schema failed. Most likely, there 
        // is an error in the schema. To debug this you can use xmllint 
        // (part of the libxml2 package).
        XML_PARSE_SCHEMA_FAILED = 205,

        // The creation of a new schema validation context failed. 
        // There might be multiple reasons for this, for example, a 
        ...
        // Abnormal internal error with sem_destroy. It should not 
        // happen! It could indicate that the memory allocated for the 
        // semaphore was corrupted (errno = EINVAL) or that the 
        // reference counting is wrong (errno = EBUSY). Please contact 
        // us.
